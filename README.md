...
Realistic Freedom Omega による画像生成API（RunPod Serverless）

## 概要

テキストプロンプトから画像を生成するAPI。ComfyUIバックエンドでRealistic Freedom Omega（SDXLベース）モデルを使用し、RunPod Serverless上で動作する。
...
| コンポーネント | 詳細 |
|--------------|------|
| 生成モデル | Realistic Freedom Omega (SDXL, 6.9GB, Civitai (要トークン)) |
| CLIP Skip | 2 |
