...
Animagine XL 4.0 による画像生成API（RunPod Serverless）

## 概要

テキストプロンプトから画像を生成するAPI。ComfyUIバックエンドでAnimagine XL 4.0（SDXLベース）モデルを使用し、RunPod Serverless上で動作する。

## 機能

- テキストから画像生成
- 自動品質タグ付与（masterpiece, high score, great score, absurdres）
...
| コンポーネント | 詳細 |
|--------------|------|
| 生成モデル | Animagine XL 4.0 (SDXL, 6.9GB, HuggingFace) |
| CLIP Skip | 2 |
