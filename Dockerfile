...
# Download PerfectDeliberate v9.0 (~6.9GB, Civitai)
ARG CIVITAI_API_TOKEN
RUN test -n "$CIVITAI_API_TOKEN" || { echo "ERROR: CIVITAI_API_TOKEN is required"; exit 1; } && \
    mkdir -p /ComfyUI/models/checkpoints && \
    wget --progress=dot:giga \
    "https://civitai.com/api/download/models/2805032?token=${CIVITAI_API_TOKEN}" \
    -O /ComfyUI/models/checkpoints/pdv99.safetensors && \
    FILESIZE=$(stat -c%s /ComfyUI/models/checkpoints/pdv99.safetensors 2>/dev/null || stat -f%z /ComfyUI/models/checkpoints/pdv99.safetensors) && \
