...
# Download Realistic Freedom Omega (~6.9GB, Civitai)
ARG CIVITAI_API_TOKEN
RUN test -n "$CIVITAI_API_TOKEN" || { echo "ERROR: CIVITAI_API_TOKEN is required"; exit 1; } && \
    mkdir -p /ComfyUI/models/checkpoints && \
    wget --progress=dot:giga \
    "https://civitai.com/api/download/models/1461059?token=${CIVITAI_API_TOKEN}" \
    -O /ComfyUI/models/checkpoints/RealisticFreedom_Omega.safetensors && \
    FILESIZE=$(stat -c%s /ComfyUI/models/checkpoints/RealisticFreedom_Omega.safetensors 2>/dev/null || stat -f%z /ComfyUI/models/checkpoints/RealisticFreedom_Omega.safetensors) && \
