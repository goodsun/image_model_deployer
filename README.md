...
Pony Diffusion V6 XL による画像生成API（RunPod Serverless）

## 概要

テキストプロンプトから画像を生成するAPI。ComfyUIバックエンドでPony Diffusion V6 XL（SDXLベース）モデルを使用し、RunPod Serverless上で動作する。
...
| コンポーネント | 詳細 |
|--------------|------|
| 生成モデル | Pony Diffusion V6 XL (SDXL, 6.5GB, HuggingFace) |
| CLIP Skip | 2 |
