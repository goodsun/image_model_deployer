...
AutismMix Pony による画像生成API（RunPod Serverless）

## 概要

テキストプロンプトから画像を生成するAPI。ComfyUIバックエンドでAutismMix Pony（SDXLベース）モデルを使用し、RunPod Serverless上で動作する。
...
| コンポーネント | 詳細 |
|--------------|------|
| 生成モデル | AutismMix Pony (SDXL, 6.5GB, HuggingFace) |
| CLIP Skip | 2 |
