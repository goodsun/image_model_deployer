...
Stable Diffusion XL Base 1.0 による画像生成API（RunPod Serverless）

## 概要

テキストプロンプトから画像を生成するAPI。ComfyUIバックエンドでStable Diffusion XL Base 1.0（SDXLベース）モデルを使用し、RunPod Serverless上で動作する。
...
| コンポーネント | 詳細 |
|--------------|------|
| 生成モデル | Stable Diffusion XL Base 1.0 (SDXL, 6.9GB, HuggingFace) |
| CLIP Skip | 2 |
